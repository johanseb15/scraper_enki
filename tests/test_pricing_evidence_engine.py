...
def c(*,market="AR",service="SOPORTE_REMOTO",conf="MEDIUM",decision=True,range_ready=True):
    return CohortePricing(
        ...
        price_scope="PER_HOUR", commercial_context="STANDARD",
    )

def test_medium_below_q1_is_bajo():
    r=evaluar_precio([c()],market="AR",canonical_service="SOPORTE_REMOTO",proposed_price_ars=Decimal("29000"),price_scope="PER_HOUR",commercial_context="STANDARD")
    assert r.status=="DECISION_READY" and r.decision_label=="BAJO"

def test_medium_iqr_is_razonable():
    r=evaluar_precio([c()],market="AR",canonical_service="SOPORTE_REMOTO",proposed_price_ars=Decimal("35000"),price_scope="PER_HOUR",commercial_context="STANDARD")
    assert r.decision_label=="RAZONABLE"

def test_medium_above_q3_is_alto():
    r=evaluar_precio([c()],market="AR",canonical_service="SOPORTE_REMOTO",proposed_price_ars=Decimal("45000"),price_scope="PER_HOUR",commercial_context="STANDARD")
    assert r.decision_label=="ALTO"

def test_low_confidence_withholds_decision():
    x=c(conf="LOW",decision=False)
    r=evaluar_precio([x],market="AR",canonical_service="SOPORTE_REMOTO",proposed_price_ars=Decimal("45000"),price_scope="PER_HOUR",commercial_context="STANDARD")
...
def test_market_mismatch_is_no_evidence():
    r=evaluar_precio([c()],market="Córdoba",canonical_service="SOPORTE_REMOTO",price_scope="PER_HOUR")
