...
def test_remote_35k_hourly_is_reasonable_for_medium_fixture():
    r=resolve("me quieren cobrar 35 lucas la hora por soporte remoto a todo el país en horario habitual, está bien?"); assert r.status=="DECISION_READY" and r.decision_label=="RAZONABLE"; assert r.evidence.observations_n==5 and r.evidence.providers_n==4

def test_remote_10k_hourly_is_low():
    r=resolve("me quieren cobrar 10 lucas la hora por soporte remoto a todo el país en horario habitual, está bien?"); assert r.status=="DECISION_READY" and r.decision_label=="BAJO"; assert r.evidence.price_position=="BELOW_OBSERVED_RANGE"

def test_remote_60k_hourly_is_high():
    r=resolve("me quieren cobrar 60 lucas la hora por soporte remoto a todo el país en horario habitual, está bien?"); assert r.status=="DECISION_READY" and r.decision_label=="ALTO"; assert r.evidence.price_position=="ABOVE_OBSERVED_RANGE"

def test_remote_query_without_explicit_national_reach_does_not_use_ar_remote_market():
    result = resolve(
        "cuánto debería cobrar por soporte remoto por hora en horario habitual?"
    )

    assert result.parsed.modality.value == "REMOTE"
    assert result.parsed.market_scope.value == "UNKNOWN"
...
def test_market_reference_hourly_returns_range_without_decision():
    ...
    r=resolve("cuánto se está cobrando por hora por soporte remoto a todo el país en horario habitual?",remote=remote); assert r.status=="RANGE_READY"; assert r.decision_label is None; assert r.evidence.median_ars==Decimal("30000")
...
def test_economic_query_still_uses_existing_pricing_flow_not_technical_readiness():
    r=resolve("me quieren cobrar 35 lucas la hora por soporte remoto a todo el país en horario habitual, está bien?")
